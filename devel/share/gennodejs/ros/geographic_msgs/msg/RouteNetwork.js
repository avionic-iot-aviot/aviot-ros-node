...
const uuid_msgs = _finder('uuid_msgs');
const BoundingBox = require('./BoundingBox.js');
const WayPoint = require('./WayPoint.js');
const RouteSegment = require('./RouteSegment.js');
...
class RouteNetwork {
  ...
  static getMessageSize(object) {
    ...
    for(let i = 0; i < object.points.length; ++i) {
      length += WayPoint.getMessageSize(object.points[i]);
    }
    for(let i = 0; i < object.segments.length; ++i) {
      length += RouteSegment.getMessageSize(object.segments[i]);
    }
    for(let i = 0; i < object.props.length; ++i) {
      length += KeyValue.getMessageSize(object.props[i]);
    }
  ...
  static datatype() {
    // Returns string type for a geographic_msgs/RouteNetwork object
